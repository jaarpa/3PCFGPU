...
)
{
    ...
    // void pcf_2iso_BPCanalytic(string histo_names, DNode *dnodeD, PointW3D *d_ordered_pointsD, int nonzero_Dnodes, int bins, int np, float size_node, float size_box, float dmax){
    ...
    time_spent=0; //Restarts timmer
    CUCHECK(cudaEventRecord(start_timmer));
    XX2iso_BPC<<<gridD,threads_perblock,0,streamDD>>>(d_DD, d_ordered_pointsD, dnodeD, nonzero_Dnodes, bins, dmax, d_max_node, size_box, size_node);
    CUCHECK(cudaMemcpyAsync(DD, d_DD, bins*sizeof(double), cudaMemcpyDeviceToHost, streamDD));
