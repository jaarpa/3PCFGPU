...
__device__ void histo_front_XX(float *PP, Node ***dat, float disn, float dn_x, float dn_y, float dn_z, bool con_in_x, bool con_in_y, bool con_in_z, int row, int col, int mom, int u, int v, int w, float size_node, int partitions, float d_max, float ds){
    int i, j, bin;
    float dis_f,dis,d_x,d_y,d_z;
    float x,y,z,w1,v;
    float size_box = partitions*size_node, dd_max=d_max*d_max;
    float ll = size_box*size_box;
    float ddmax_nod = dmax + size_node*sqrt(3.0);
...
__global__ void make_histoXX(float *XX, Node ***nodeD, int partitions, int bn, float dmax, float size_node, int start_at){
    ...
    if (idx<(partitions*partitions*partitions)){
        ...
        float d_max_pm = dmax + size_node*0.5, front_pm = (size_node*partitions - d_max) - size_node*0.5;
        ...
        if (nodeD[row][col][mom].len > 0){
            ...
            for(w = mom+1; w<partitions; w++){
                ...
                if(con_z){
                histo_front_XX(XX,nodeD,dd_nod12,0.0,0.0,fabs(dz_nod12),false,false,con_z,row,col,mom,u, v,w);
                }
            }

            //Second node mobil in YZ
            for(v=col+1; v<partitions; v++){
                ny2 = nodeD[u][v][0].nodepos.y;
                dy_nod12 = ny2 - ny1;
                for(w=0; w<partitions; w++){
                    ...
                    con_z = ((nz1<=d_max_pm)&&(nz2>=front_pm))||((nz2<=d_max_pm)&&(nz1>=front_pm));
                    if(con_y || con_z){ 
                    histo_front_XX(XX,nodeD,dd_nod12,0.0,sqrt(dy_nod12),sqrt(dz_nod12),false,con_y,con_z,row,col,mom,u,v,w);
                    }
                }
            }
            ...
            for(u = row+1; u < partitions; u++){
                ...
                for(v = 0; v < partitions; v++){
                    ...
                    for(w = 0; w < partitions; w++){
                        ...
                        histo_front_XX(XX,nodeD,dd_nod12,sqrt(dx_nod12),sqrt(dy_nod12),sqrt(dz_nod12),con_x,con_y,con_z,row,col,mom,u,v,w);
                        }
                    }
                }
            }
