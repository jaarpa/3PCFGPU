...
// nvcc -arch=sm_75 main.cu -o par_s.out && ./par_s.out data.dat rand0.dat 32768 10 150
...
int main(int argc, char **argv){
    ...
    with the names DD.dat DR.dat RR.dat. This program do not consider periodic boundary conditions. The data files must contain 4 columns, the first 3 
    are the x,y,z coordinates and the 4 the weight of the measurment.

    Args:
    arg[1]: name or path to the data file relative to ../../../data/. 
    arg[2]: name or path to the random file relative to ../../../data/
    arg[3]: integer of the number of points in the files.
    arg[4]: integer. Number of bins where the distances are classified. The resulting histagram is a bn X bn size array.
    ...
    */
    ...
    string nameDD = "DDani.dat", nameRR = "RRani.dat", nameDR = "DRani.dat";
    ...
    make_histoXX<<<gridD,threads_perblock_D,0,streamDD>>>(d_DD, d_ordered_pointsD_DD, dnodeD_DD, partitions, bn, dmax, size_node);
    make_histoXX<<<gridR,threads_perblock_R,0,streamRR>>>(d_RR, d_ordered_pointsR_RR, dnodeR_RR, partitions, bn, dmax, size_node);
    make_histoXY<<<gridDR,threads_perblock_DR,0,streamDR>>>(d_DR, d_ordered_pointsD_DR, dnodeD_DR, d_ordered_pointsR_DR, dnodeR_DR, partitions, bn, dmax, size_node);
