...
__global__ void make_histo_analitic(double *XXY, double *RRR, double *ff_av, double *ff_av_ref, float dmax, int bn, int bn_ref){
    /*
    */
    ...
    if (i<bn && j<bn && k<bn){
        ...
        if (v_in==8){
            ...
            if (con){
                ...
                atomicAdd(&RRR[i][j][k],S_av);
                f_av *= gama*dr_ref*dr_ref*dr_ref;
                atomicAdd(&XXY[i][j][k],f_av);
            }
        }
    }
}