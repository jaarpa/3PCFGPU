...
do 
	...
	do
			3PCFisotropic/normal/opt.out data_512MPc.dat 50653 20 $dmax $partitions
	done	
done
...
do 
	for ((partitions=20;partitions<=60;partitions+=5))
	do
			3PCFisotropic/normal/opt.out data_1GPc.dat 50653 20 $dmax $partitions
	done	
done
...
do 
	for ((partitions=20;partitions<=60;partitions+=5))
	do
			3PCFisotropic/normal/opt.out data_2GPc.dat 50653 20 $dmax $partitions
	done	
done
